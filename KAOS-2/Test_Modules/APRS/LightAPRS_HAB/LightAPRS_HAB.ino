...
char comment[50] = "KAOS-2 HAB"; // Max 50 char
char StatusMessage[50] = "KAOS-2 LightAPRS"; 
boolean txEnabled = false; // Bench mode: set true only with licensed callsign and approved APRS settings.
//*****************************************************************************
...
void loop() {
  ...
  if (readBatt() > BattMin) {
    if (aliveStatus) {
#if defined(DEVMODE)
      Serial.println(F("Startup status requested"));
#endif
      if (txEnabled) {
        sendStatus();
#if defined(DEVMODE)
        Serial.println(F("Startup status sent"));
#endif
      } else {
#if defined(DEVMODE)
        Serial.println(F("TX disabled: startup status not sent"));
#endif
      }
      aliveStatus = false;
    }
      ...
      if (gps.satellites.isValid() && (gps.satellites.value() > 3)) {
        ...
        if (gps.time.minute() == 30) {
          if (txEnabled) {
            sendStatus();
#if defined(DEVMODE)
            Serial.println(F("Scheduled status sent"));
#endif
          } else {
#if defined(DEVMODE)
            Serial.println(F("TX disabled: scheduled status not sent"));
#endif
          }
        } else {
          if (txEnabled) {
...
#if defined(DEVMODE)
            Serial.println(F("TX disabled: location not sent"));
#endif
          }
        }
