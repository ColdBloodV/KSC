// KAOS-2 firmware based on lightaprs/LightAPRS-2.0 commit 61ea7b6.
#include <Arduino.h>
#include <Wire.h>
#include <math.h>
#include <stdio.h>
...
#define DEVMODE // Comment out only after flight verification is complete.

#define PICO_LINK_ENABLED 1
#define PICO_I2C_ADDRESS 0x42

#define RF_TEST_MODE 1
#define RF_TEST_INTERVAL_MS 60000UL
...
char    comment[50] = "KAOS-2 HAB"; // Max 50 char but shorter is better
char    StatusMessage[50] = "KAOS-2 LightAPRS 2.0";
boolean txEnabled = true;
//*****************************************************************************

uint16_t  BeaconWait=60;  //seconds until the next APRS location beacon
 ...
 */
uint8_t pathSize=1; // WIDE2-1 only for the airborne payload
...
uint16_t rfTestCount = 1;
unsigned long lastRfTestMs = 0;

// GPS-only landing fallback. APRS requests shutdown; the Pico handles the Pi
// shutdown and power sequencing. APRS remains powered after the request.
const unsigned long MIN_FLIGHT_TIME_MS = 4UL * 60UL * 60UL * 1000UL;
const unsigned long LANDING_CONFIRM_MS = 2UL * 60UL * 1000UL;
const unsigned long SHUTDOWN_RETRY_MS = 60UL * 1000UL;
const float LANDING_ALTITUDE_M = 50.0f;
unsigned long landingBelowSinceMs = 0;
unsigned long lastShutdownAttemptMs = 0;
...
void setup() {
  ...
  // ZeroAPRS transmits PATH1 when pathSize is 1.
  APRS_setPath1("WIDE2", Wide2);
  ...
  if (bmp.begin()) {
    SerialUSB.println(F("[BMP180] OK"));
  } else {
    SerialUSB.println(F("[BMP180] FAIL"));
  }
    ...
    if (aliveStatus) {
      ...
      if (myGPS.getPVT()) {
        ...
        if ( (myGPS.getFixType() != 0) && (myGPS.getSIV() > 3) ) {
          ...
          sendRfTestStatusIfDue();
          delay(1000);
        }
      } else {
        #if defined(DEVMODE)
        SerialUSB.println(F("No new GPS PVT data"));
        #endif
        sendRfTestStatusIfDue();
        delay(1000);
      }
...
void sendGpsToPico() {
#if PICO_LINK_ENABLED
  long latitudeE5 = myGPS.getLatitude() / 100L;
  long longitudeE5 = myGPS.getLongitude() / 100L;
  long altitudeM = myGPS.getAltitude() / 1000L;
  unsigned int satellites = myGPS.getSIV();

  char packet[40];
  snprintf(packet, sizeof(packet), "G,%ld,%ld,%ld,%u,1",
           latitudeE5, longitudeE5, altitudeM, satellites);

  Wire.beginTransmission(PICO_I2C_ADDRESS);
  Wire.write((const uint8_t *)packet, strlen(packet));
  byte result = Wire.endTransmission();
  if (result != 0) {
    SerialUSB.print(F("Pico I2C GPS send failed: "));
    SerialUSB.println(result);
  }
#endif
}

void checkLandingShutdown() {
#if PICO_LINK_ENABLED
  if (shutdownSent || millis() < MIN_FLIGHT_TIME_MS) return;

  float altitudeM = myGPS.getAltitude() / 1000.0f;
  if (altitudeM > LANDING_ALTITUDE_M) {
    landingBelowSinceMs = 0;
    return;
  }

  if (landingBelowSinceMs == 0) {
    landingBelowSinceMs = millis();
    return;
  }
  if (millis() - landingBelowSinceMs < LANDING_CONFIRM_MS) return;
  if (lastShutdownAttemptMs != 0 &&
      millis() - lastShutdownAttemptMs < SHUTDOWN_RETRY_MS) return;

  lastShutdownAttemptMs = millis();
  const char packet[] = "SHUTDOWN";
  Wire.beginTransmission(PICO_I2C_ADDRESS);
  Wire.write((const uint8_t *)packet, strlen(packet));
  byte result = Wire.endTransmission();
  if (result == 0) {
    shutdownSent = true;
    SerialUSB.println(F("Landing confirmed: shutdown sent to Pico"));
  } else {
    SerialUSB.print(F("Landing confirmed, Pico I2C shutdown failed: "));
    SerialUSB.println(result);
  }
#endif
}

void sendRfTestStatusIfDue() {
#if RF_TEST_MODE
  unsigned long now = millis();
  if (lastRfTestMs != 0 && now - lastRfTestMs < RF_TEST_INTERVAL_MS) return;
  lastRfTestMs = now;

  char testMessage[32];
  if (myGPS.getYear() >= 2020 && myGPS.getMonth() > 0 && myGPS.getDay() > 0) {
    snprintf(testMessage, sizeof(testMessage), "%02u%02u%02uz TEST %u",
             (unsigned int)myGPS.getDay(),
             (unsigned int)myGPS.getHour(),
             (unsigned int)myGPS.getMinute(),
             rfTestCount++);
  } else {
    snprintf(testMessage, sizeof(testMessage), "000000z TEST %u", rfTestCount++);
  }

  SerialUSB.print(F("RF test status sending: "));
  SerialUSB.println(testMessage);
  if (!txEnabled) {
    SerialUSB.println(F("RF test status skipped: TX disabled"));
    return;
  }
  ...
  PttON;
  delay(1000);
  APRS_sendStatus(testMessage);
  delay(10);
  PttOFF;
  RadioOFF;
  delay(1000);
  SerialUSB.println(F("RF test status sent"));
  TxCount++;
#endif
}
...
void sendLocation() {
  if (!txEnabled) {
    SerialUSB.println(F("Location skipped: TX disabled"));
    return;
  }
...
void sendStatus() {
  if (!txEnabled) {
    SerialUSB.println(F("Status skipped: TX disabled"));
    return;
  }
